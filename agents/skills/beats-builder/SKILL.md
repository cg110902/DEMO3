...
- 任务内给的材料（见上）；可另读 `craft#视角与信息差`、`craft#反公式化与拟人化` 取标准。
- `state/lines.json` 里挂在本书账上的线（哪些本章必须动）。

## 动作

1. 定 form：对照 craft 章型库与主控给的"上一章 form"；同卷分布数据在
...
4. 写 front-matter：只放合法键（见 `craft#front-matter 键`），form/pov/words/style_notes 齐全。
5. 章尾方式在 style_notes 里写明（四选一见 `craft#钩子与爽感节奏`）。
...
写 entities 提案）；不写正文样句（会传染起草）；不复述 craft 原文当理由。
