...
> 定位：v1.1（初版，卷一 6 章生产 + 两轮自查修复）之上的升级提案。**本文不是生产指令**：
> 每项经主控与用户裁决后，改动先写入 PLAN 相应节（含附录 A 白名单增补），再动码。
...
- **E3 分景起草协议（纯文档）**：任务书「目标」按场景分块并各配字数带建议；起草代理逐景写、
  ...
  引擎零改动。
- **E4 pack 输出末尾自动附"本章还欠什么"一行**（gaps 快到期 + 待处理 failed/ 件数）——
  主控 Stage 1 少跑一次命令。若嫌夹带可裁（争议：pack 应保持纯上下文）。
